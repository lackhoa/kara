...
; (Implicit predicate)
; (Explicit set)
; Actually, empty molecules can also be considered atoms. Atoms and molecules are also called "aggregates", as they represent possibilities.
...
(def (atom-intersect atom1 atom2)
  ...
  (when (or (and (typed-mole? atom1)
                 (not (mole-empty? atom1)))
            (and (typed-mole? atom2)
...
(def (typed-mole? exp) (tagged? exp 'Mole))
